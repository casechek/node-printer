const fs = require("fs"), child_process = require("child_process"), os = require("os"), path = require("path"),
    binding_path = path.resolve(__dirname, './node_printer.node');
let printer_helper;

if (!fs.existsSync(binding_path)) {
    throw new Error('@casechek/node-printer: native addon not found at ' + binding_path + '. ' + 'Build it with `npm rebuild @casechek/node-printer`. This requires a C++20 ' + 'toolchain -- Xcode Command Line Tools on macOS, Visual Studio 2022 or ' + 'newer Build Tools on Windows.');
}
...
function printDirect(parameters) {
    var data = parameters, printer, docname, type, options, success, error;
